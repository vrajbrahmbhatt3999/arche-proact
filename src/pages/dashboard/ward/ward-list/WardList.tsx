...
import { medicalCenterWardTableHeader } from "../../../../constants/data";
import { useAppDispatch, useAppSelector } from "../../../../hooks";
import { requestGenerator } from "../../../../utils/payloadGenerator";
import styles from "./wardList.module.scss";
...
import { UPDATE_WARD_STATUS } from "../../../../constants/asyncActionsType";
...
const WardList = () => {
  ...
  const { isLoading, wardData } = useAppSelector((state) => state.ward);
  ...
  useEffect(() => {
    ...
    dispatch(getAllWard(requestGenerator(data))).then((result) => {
  ...
  const handleStatus = () => {
    ...
    dispatch(updateWardStatus(requestGenerator(statusPayload))).then((e) => {
      if ((e.type = `${UPDATE_WARD_STATUS}/fulfilled`)) {
    ...
    <>
      ...
      {confirm && (
        <Popup
          ...
          message="Are you sure you want to change the selected ward status?"
        />
      )}
      <CustomModal
        showModal={showNotes}
      ...
      <div className={styles.branchListContainer}>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value === "") {
                    ...
                    dispatch(getAllWard(requestGenerator(data))).then(
                      (result) => {
                        setTotalPage(result.payload.lastPage);
                        setTotalData(result.payload.total);
                      }
        ...
        <Table
          tableHeaderData={medicalCenterWardTableHeader}
          tableRowData={wardData}
